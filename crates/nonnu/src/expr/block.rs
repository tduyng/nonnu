use crate::{
    env::Env,
    stmt::Stmt,
    utils::{extract_whitespace, remove_tag},
    val::Val,
};
...
impl Block {
    pub fn new(s: &str) -> Result<(&str, Self), String> {
        let s = remove_tag("{", s)?;
        let (mut s, _) = extract_whitespace(s, None)?;
        let mut stmts = Vec::new();

        while let Ok((new_s, stmt)) = Stmt::new(s) {
            s = new_s;
            stmts.push(stmt);
            let (new_s, _) = extract_whitespace(s, None)?;
            s = new_s;
        }
...
mod tests {
    ...
    #[test]
    fn eval_empty_block() {
        assert_eq!(Block { stmts: Vec::new() }.eval(&Env::default()), Ok(Val::Unit),);
    }
