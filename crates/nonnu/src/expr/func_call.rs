use crate::{
    utils::{extract_ident, extract_while, sequence},
    Env, Val,
};
...
impl FuncCall {
    pub fn new(s: &str) -> Result<(&str, Self), String> {
        let (s, callee) = extract_ident(s)?;
        let (s, _) = extract_while(|c| c == ' ', s, None)?;

        let (s, params) = sequence(
            Expr::new,
            |s, error_msg| {
                extract_while(|c| c == ' ', s, error_msg.map(|msg| msg.to_owned()))
                    .map(|(new_s, _)| (new_s, Expr::new(s).map(|(_, expr)| expr).unwrap()))
            },
            s,
            None,
        )?;
...
mod tests {
    ...
    fn eval_func_call() {
        ...
        env.store_func(
            "id".to_string(),
            vec!["x".to_string()],
            Stmt::Expr(Expr::BindingUsage(BindingUsage { name: "x".to_string() })),
        );
    ...
    fn eval_func_call_with_too_few_parameters() {
        ...
        env.store_func(
            ...
            Stmt::Expr(Expr::Operation {
                ...
                rhs: Box::new(Expr::BindingUsage(BindingUsage { name: "b".to_string() })),
                op: Op::Mul,
            }),
        );
    ...
    fn eval_func_call_with_too_many_parameters() {
        ...
        env.store_func(
            ...
            vec!["n".to_string()],
            Stmt::Expr(Expr::Operation {
                lhs: Box::new(Expr::BindingUsage(BindingUsage { name: "n".to_string() })),
                rhs: Box::new(Expr::BindingUsage(BindingUsage { name: "n".to_string() })),
                op: Op::Mul,
            }),
        );
