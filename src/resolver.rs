...
impl Resolver<'_> {
	fn new(index: &indexer::Index) -> Resolver<'_> {
		Resolver { index, procedures: IndexMap::new(), named_tys: IndexMap::new(), in_progress_tys: Vec::new() }
	}
	...
	fn resolve_procedure(&mut self, name: &str, procedure: &indexer::Procedure) {
		...
		for parameter in &procedure.parameters {
			parameters.push(Parameter { name: parameter.name.clone(), ty: self.resolve_ty(&parameter.ty) });
		}
	...
	fn resolve_ty_definition(&mut self, name: &str, ty_definition: &indexer::TyDefinition) {
		match ty_definition {
			indexer::TyDefinition::Struct(struc) => {
			...
			indexer::TyKind::Named(n) => {
				...
				if !self.named_tys.contains_key(&n) {
					if self.in_progress_tys.contains(&n) {
						crate::error(ty.loc.clone(), "detected cycle in type definition".to_string());
					}
...
fn tests() {
	...
	crate::testing::run_tests("tests/resolver", |input| {
