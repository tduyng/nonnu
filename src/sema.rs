	...
	fn analyze_statement(&mut self, statement: &ast::Statement) -> Option<Idx<Statement>> {
		let s = match &statement.kind {
			...
			ast::StatementKind::If { condition, true_branch, false_branch } => {
				...
				let false_branch_idx = false_branch.as_ref().map(|s| self.analyze_statement(s).unwrap_or(empty_branch));
			...
			ast::StatementKind::Return { value } => match (&self.procedure.return_ty, value) {
				(Some(return_ty), Some(value)) => {
					...
					if value_ty != return_ty {
						crate::error(
							value.loc.clone(),
							format!("cannot return “{value_ty}” from procedure which returns “{return_ty}”"),
						);
					}
				...
				(Some(return_ty), None) => crate::error(
					statement.loc.clone(),
					format!("cannot return without value from procedure which returns “{return_ty}”"),
				),

				(None, Some(value)) => crate::error(
					value.loc.clone(),
					"cannot return with value from procedure which does not return value".to_string(),
				),
...
fn acceptable_tys_for_operator(operator: ast::BinaryOperator) -> &'static [Ty] {
	match operator {
		...
		ast::BinaryOperator::BitAnd | ast::BinaryOperator::BitOr | ast::BinaryOperator::BitXor => &[Ty::Int, Ty::Bool],
...
fn tests() {
	...
	crate::testing::run_tests("tests/sema", |input| {
