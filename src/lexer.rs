...
impl Lexer<'_> {
	fn new(text: &str, file: PathBuf) -> Lexer<'_> {
		Lexer { text, bytes: text.as_bytes(), i: 0, loc: Loc { line: 1, column: 1, file }, tokens: Vec::new() }
	}
	...
	fn recognize(&mut self, options: &[(&[u8], TokenKind)]) -> bool {
		...
		for (prefix, kind) in options {
			if self.bytes[self.i..].starts_with(prefix) {
				...
				self.tokens.push(Token { kind: *kind, text: self.text[start..self.i].to_string(), loc });
...
fn tests() {
	...
	crate::testing::run_tests("tests/lexer", |input| {
